...
import "file:/usr/share/omarchy/shell/services" as Services
...
Item {
  ...
  function loadProvidersForSearch() {
    ...
    var inApps = active === "apps" || root.isDescendantOf(active, "apps")

    for (var i = 0; i < root.itemOrder.length; i++) {
      var entry = root.item(root.itemOrder[i])
      if (!entry || !entry.provider || root.providersLoaded[entry.id]) continue
      if (entry.id === "apps" && !inApps) continue
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      for (var i = 0; i < root.itemOrder.length; i++) {
        ...
        if (!inApps && (entry.kind === "app" || entry.parent === "apps" || root.isDescendantOf(entry.id, "apps"))) continue
        if (!root.isDescendantOf(entry.id, active)) continue
        if (!root.matchesQuery(entry, queryTerms)) continue
    ...
    for (var k = 0; k < limit; k++) displayModel.append(rows[k])
    layoutSerial += 1
  ...
  function openExistingMenu(initialMenu) {
    ...
    cursorActive = true
    root.disarmPointer()
    if (!root.whenResults || Object.keys(root.whenResults).length === 0) {
      root.evaluateGuards()
    }
    opened = true
    rebuildDisplay()
    ...
    // Avoid re-running an expensive 10k-file icon scan on every single menu open
    if (root.appLibrary && (!root.appLibrary.iconIndex || Object.keys(root.appLibrary.iconIndex).length === 0)) {
      ...
      Item {
        ...
        Keys.onEscapePressed: function(event) {
          if (root.deleteConfirmOpen) {
            if (deleteConfirm.handleKey(event)) { event.accepted = true; return }
          }
          root.cancel()
          event.accepted = true
        }
