...
function nameSearchText(entry) {
  if (!entry) return ""
  if (entry._searchText !== undefined) return entry._searchText
  var aliases = []
...
function descriptionTextMatches(query, text) {
  var terms = Array.isArray(query) ? query : String(query || "").toLowerCase().trim().split(/\s+/)
...
function searchScore(items, entry, query) {
  ...
  if (entry._lowerLabel === undefined) entry._lowerLabel = String(entry.label || "").toLowerCase()
  var label = entry._lowerLabel
  var nameText = nameSearchText(entry)
  var descriptionText = String(entry.description || "").toLowerCase()
  var score = 80
